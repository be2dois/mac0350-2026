...
async def create_user(usuario: Usuario):
    ...
    return f"Usuário Adicionado!<br>Nome: {usuario.nome}<br>Idade: {usuario.idade}"
...
async def get_users(index: int | None = None):
    if index is not None:
        if 0 <= index < len(usuarios):
            ...
            return f"Nome: {usuario.nome}<br>Idade: {usuario.idade}"
        return "Usuário não encontrado."
    todos = ""
    for i in range(len(usuarios)):
        todos += f"Usuário {str(i)} - Nome: {usuarios[i].nome} - Idade: {usuarios[i].idade}<br>"
    return todos
 
@app.delete("/users", response_class=PlainTextResponse)
